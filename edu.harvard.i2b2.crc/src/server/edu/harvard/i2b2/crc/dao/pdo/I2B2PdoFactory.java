...
public class I2B2PdoFactory {
	...
	public class ObservationFactBuilder {
		/** detail flag **/
		boolean obsFactDetailFlag;
		/** blob flag **/
		boolean obsFactBlobFlag;
		/** status flag **/
		boolean obsFactStatusFlag;
		 ...
		 * @throws IOException
		 */
		public ObservationType buildObservationSet(ResultSet rowSet) throws SQLException, IOException {
			...
			if (startDate != null)
				observationFactType.setStartDate(dtoFactory.getXMLGregorianCalendar(startDate.getTime()));

			ObservationType.ObserverCd observerCd = new ObservationType.ObserverCd();
			observerCd.setValue(((rowSet.getString("obs_provider_id") != null)
			...
			if (obsFactDetailFlag) {
				...
				if (endDate != null)
					observationFactType.setEndDate(dtoFactory.getXMLGregorianCalendar(endDate.getTime()));

				observationFactType.setValuetypeCd(rowSet.getString("obs_valtype_cd"));
				observationFactType.setTvalChar((rowSet
						.getString("obs_tval_char") != null) ? rowSet.getString("obs_tval_char") : "");
				...
				observationFactType.setConfidenceNum(rowSet.getBigDecimal("obs_confidence_num"));
			...
			if (obsFactStatusFlag) {
				if (rowSet.getTimestamp("obs_update_date") != null)
					observationFactType.setUpdateDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("obs_update_date").getTime()));

				if (rowSet.getTimestamp("obs_download_date") != null)
					observationFactType.setDownloadDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("obs_download_date").getTime()));

				if (rowSet.getTimestamp("obs_import_date") != null)
					observationFactType.setImportDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("obs_import_date").getTime()));

				observationFactType.setSourcesystemCd(rowSet.getString("obs_sourcesystem_cd"));
				observationFactType.setUploadId(rowSet.getString("obs_upload_id"));
			}
	...
	public class PatientBuilder {
		boolean patientDetailFlag;
		boolean patientBlobFlag;
		boolean patientStatusFlag;
		String dbType;
		 ...
		 * @throws IOException
		 */
		public PatientType buildPatientSet(ResultSet rowSet, List<ParamType> metaDataParamList) throws SQLException, IOException {
			...
			if (patientStatusFlag)
				if (rowSet.getTimestamp("patient_update_date") != null) {
					patientDimensionType.setUpdateDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("patient_update_date").getTime()));

				if (rowSet.getTimestamp("patient_download_date") != null)
					patientDimensionType.setDownloadDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("patient_download_date").getTime()));

				if (rowSet.getTimestamp("patient_import_date") != null)
					patientDimensionType.setImportDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("patient_import_date").getTime()));

				patientDimensionType.setSourcesystemCd(rowSet.getString("patient_sourcesystem_cd"));
				patientDimensionType.setUploadId(rowSet.getString("patient_upload_id"));
			}
	...
	public class ProviderBuilder {
		boolean providerDetailFlag;
		boolean providerBlobFlag;
		boolean providerStatusFlag;
		String dbType;
		 ...
		 * @throws IOException
		 */
		public ObserverType buildObserverSet(ResultSet rowSet) throws SQLException, IOException {
			...
			providerDimensionType.setObserverPath(rowSet.getString("provider_provider_path"));
			...
			if (providerStatusFlag) {
				if (rowSet.getTimestamp("provider_update_date") != null)
					providerDimensionType.setUpdateDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("provider_update_date").getTime()));

				if (rowSet.getTimestamp("provider_download_date") != null)
					providerDimensionType.setDownloadDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("provider_download_date").getTime()));

				if (rowSet.getTimestamp("provider_import_date") != null)
					providerDimensionType.setImportDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("provider_import_date").getTime()));

				providerDimensionType.setSourcesystemCd(rowSet.getString("provider_sourcesystem_cd"));
				providerDimensionType.setUploadId(rowSet.getString("provider_upload_id"));
			}
	...
	public class ConceptBuilder {
		boolean conceptDetailFlag;
		boolean conceptBlobFlag;
		boolean conceptStatusFlag;
		String dbType;
		/**
		 ...
		 * @throws IOException
		 */
		public ConceptType buildConceptSet(ResultSet rowSet) throws SQLException, IOException {
			...
			if (conceptDetailFlag) {
				conceptDimensionType.setConceptCd(rowSet.getString("concept_concept_cd"));
				conceptDimensionType.setConceptPath(rowSet.getString("concept_concept_path"));
			...
			if (conceptStatusFlag) {
				if (rowSet.getTimestamp("concept_update_date") != null)
					conceptDimensionType.setUpdateDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("concept_update_date").getTime()));

				if (rowSet.getTimestamp("concept_download_date") != null)
					conceptDimensionType.setDownloadDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("concept_download_date").getTime()));

				if (rowSet.getTimestamp("concept_import_date") != null)
					conceptDimensionType.setImportDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("concept_import_date").getTime()));

				conceptDimensionType.setSourcesystemCd(rowSet.getString("concept_sourcesystem_cd"));
				conceptDimensionType.setUploadId(rowSet.getString("concept_upload_id"));
			}
	...
	public class ModifierBuilder {
		...
		boolean modifierDetailFlag;
		boolean modifierBlobFlag;
		boolean modifierStatusFlag;
		String dbType;
		 ...
		 * @throws IOException
		 */
		public ModifierType buildModifierSet(ResultSet rowSet) throws SQLException, IOException {
			ModifierType modifierDimensionType = new ModifierType();
			modifierDimensionType.setModifierCd(rowSet.getString("modifier_modifier_cd"));

			if (modifierDetailFlag) {
				modifierDimensionType.setModifierCd(rowSet.getString("modifier_modifier_cd"));
				modifierDimensionType.setModifierPath(rowSet.getString("modifier_modifier_path"));
				modifierDimensionType.setNameChar(rowSet.getString("modifier_name_char"));
			}
			...
			if (modifierStatusFlag) {
				if (rowSet.getTimestamp("modifier_update_date") != null)
					modifierDimensionType.setUpdateDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("modifier_update_date").getTime()));

				if (rowSet.getTimestamp("modifier_download_date") != null)
					modifierDimensionType.setDownloadDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("modifier_download_date").getTime()));

				if (rowSet.getTimestamp("modifier_import_date") != null)
					modifierDimensionType.setImportDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("modifier_import_date").getTime()));

				modifierDimensionType.setSourcesystemCd(rowSet.getString("modifier_sourcesystem_cd"));
				modifierDimensionType.setUploadId(rowSet.getString("modifier_upload_id"));
			}
	...
	public class EventBuilder {
		boolean eventDetailFlag;
		boolean eventBlobFlag;
		boolean eventStatusFlag;
		String dbType;
		 ...
		 * @throws IOException
		 */
		public EventType buildEventSet(ResultSet rowSet,List<ParamType> metaDataParamList) throws SQLException, IOException {
			...
			if (eventDetailFlag) {
				...
				if (startDate != null)
					visitDimensionType.setStartDate(dtoFactory.getXMLGregorianCalendar(startDate.getTime()));

				Date endDate = rowSet.getTimestamp("visit_end_date");

				if (endDate != null)
					visitDimensionType.setEndDate(dtoFactory.getXMLGregorianCalendar(endDate.getTime()));

				for (Iterator<ParamType> metaParamIterator = metaDataParamList.iterator(); metaParamIterator.hasNext();) { 
					ParamType metaParamType = metaParamIterator.next();
					ParamTypeValueBuilder paramValBuilder = new ParamTypeValueBuilder();
					visitDimensionType.getParam().add(paramValBuilder.buildParamType(metaParamType,
							"visit_",null,rowSet));
				}
			}
			...
			if (eventStatusFlag) {
				if (rowSet.getTimestamp("visit_update_date") != null)
					visitDimensionType.setUpdateDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("visit_update_date").getTime()));

				if (rowSet.getTimestamp("visit_download_date") != null)
					visitDimensionType.setDownloadDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("visit_download_date").getTime()));

				if (rowSet.getTimestamp("visit_import_date") != null)
					visitDimensionType.setImportDate(dtoFactory
							.getXMLGregorianCalendar(rowSet.getTimestamp("visit_import_date").getTime()));

				visitDimensionType.setSourcesystemCd(rowSet.getString("visit_sourcesystem_cd"));
				visitDimensionType.setUploadId(rowSet.getString("visit_upload_id"));
			}
