...
public class QueryResultEncounterSetGenerator extends CRCDAO implements IResultGenerator {
	...
	public void generateResult(Map param) throws I2B2DAOException {
		...
		try {
			...
			queryGeneratorVersion = qpUtil.getCRCPropertyValue(
					"edu.harvard.i2b2.crc.setfinder.querygenerator.version");
		...
		try {
			int i = 0;
			IEncounterSetCollectionDao encounterSetCollectionDao = sfDAOFactory.getEncounterSetCollectionDAO();
			encounterSetCollectionDao.createPatientEncCollection(resultInstanceId);
			...
			sql = "INSERT INTO " + dbSchemaName + "qt_patient_enc_collection"
					...
					+ "SELECT ? AS result_instance_id, %VID AS set_index, t.patient_num, t.encounter_num "
					+ "FROM (" + encounterSql + ") t order by t.patient_num,t.encounter_num";
			log.debug("Script: " + sql);
			...
			exception = sqlEx;
			log.error("QueryResultEncounterSetGenerator.generateResult:" + sqlEx.getMessage(), sqlEx);
			throw new I2B2DAOException("QueryResultEncounterSetGenerator.generateResult:" + sqlEx.getMessage(), sqlEx);
	...
	private String buildEncounterSetSql(SetFinderDAOFactory sfDAOFactory,
		...
		if (queryGeneratorVersion.equals("1.6") || queryGeneratorVersion.equals("1.7")) {
			...
			String queryMasterId = queryInstance.getQtQueryMaster().getQueryMasterId();
			IQueryMasterDao queryMasterDao = sfDAOFactory.getQueryMasterDAO();
			QtQueryMaster queryMasterType = queryMasterDao.getQueryDefinition(queryMasterId);
			QueryDefinitionUnWrapUtil qpUnwrapUtil = new QueryDefinitionUnWrapUtil();
			QueryDefinitionType queryDefType;
			try {
				queryDefType = qpUnwrapUtil.getQueryDefinitionType(queryMasterType.getI2b2RequestXml());
			} catch (I2B2DAOException e) {
				throw e;
			}
			QueryTimingHandler queryTimingHandler = new QueryTimingHandler();
			boolean sameVisitFlag = queryTimingHandler.isSameVisit(queryDefType);
