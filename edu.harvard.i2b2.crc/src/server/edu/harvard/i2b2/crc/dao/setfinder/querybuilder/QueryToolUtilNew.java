...
public class QueryToolUtilNew extends CRCDAO {
	...
	public String getSetfinderSqlForQueryDefinition() throws JAXBUtilException, I2B2Exception {
		...
		QueryDefinitionType queryDefType = queryDefUnWrapHelper.getQueryDefinitionType(this.queryXML);
		
		boolean invertFlag = isInvert(queryDefType);
		boolean onlyInvertFlag = isOnlyInvert(queryDefType);
		if (onlyInvertFlag) {
			BuildInvertSql buildInvertSql = new BuildInvertSql(dataSourceLookup, this.getTempTableName());
		...
		SecurityType securityType = PMServiceAccountUtil.getServiceSecurityType(tempSecurityType.getDomain());
		...
		Map<String, List<PanelType>> grpPanelByTimingMap = grpPanelTiming
				.groupPanelByTiming(queryDefType.getPanel(), queryDefType.getQueryTiming());
		...
		possibleMap.put("SAMEINSTANCENUM", StringUtils.EMPTY);
		...
		String[] timingOrder = new String[] {QueryTimingHandler.SAMEINSTANCENUM,
		...
		for (Map<String, List<PanelType>> panelGroupByInvertList  : t) { 
			for (int k = 0; k < timingOrder.length; k++) {
				...
				if (groupedPanelList != null && groupedPanelList.size()>0) {
					if (timingOrder[k].equalsIgnoreCase(QueryTimingHandler.SAMEINSTANCENUM)) {
						instanceNumFlag = true;
						encounterFlag = false;
					} else if (timingOrder[k].equalsIgnoreCase(QueryTimingHandler.SAMEVISIT)) {
						encounterFlag = true;
						instanceNumFlag = false;
					} else if (timingOrder[k].equalsIgnoreCase(QueryTimingHandler.ANY)) {
						encounterFlag = false;
						instanceNumFlag = false;
					}
		...
		DxTableHandler dxTableHandler = new DxTableHandler(this.getDbSchemaName(), this.getTempDxTableName(), this.getTempTableName());
		String dxTempTableSql = dxTableHandler.buildDxInsertSql(encounterFlag, instanceNumFlag, maxPanelNum);
		setfinderSql.append(dxTempTableSql);
		log.debug(setfinderSql.toString());
		return setfinderSql.toString();
	}

	private Map<Integer, String> generateSqlForGroupedPanelList(List<PanelType> panelList, QueryDefinitionType queryDefType,
																SecurityType securityType, String projectId, boolean encounterFlag,
																boolean instanceNumFlag, int panelCount, boolean firstPanelFlag,
																boolean invertQueryFlag, boolean invertOnlyQueryFlag,
																String firstItemJoinTiming) throws I2B2Exception {
		SortPanel sortPanel = new SortPanel();
		LogTimingUtil timingUtil = new LogTimingUtil();
		timingUtil.setStartTime();
		List<PanelType> sortedPanelList = sortPanel.sortedPanelList(panelList, securityType, projectId);
		...
		BuildTempTableSql tempTableSql = new BuildTempTableSql( dataSourceLookup, queryXML);
		...
		this.ignoredItemMessageBuffer.append(tempTableSql.getIgnoredItemMessage());
		return panelSql;
	}
	...
	private String calculateFirstItemJoin(String[] timingOrder, Map<String, List<PanelType>> invertGroupPanelByTimingMap,
										  Map<String, List<PanelType>> nonInvertGroupPanelByTimingMap) {
		...
		if (invertTiming.equalsIgnoreCase(QueryTimingHandler.SAMEINSTANCENUM)) { 
			if (nonInvertTiming.equalsIgnoreCase(QueryTimingHandler.ANY)
					|| nonInvertTiming.equalsIgnoreCase(QueryTimingHandler.SAMEVISIT))
