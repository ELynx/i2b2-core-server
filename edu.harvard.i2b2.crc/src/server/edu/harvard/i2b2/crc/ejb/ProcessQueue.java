...
//	private boolean isActive = false;
	...
	// If queued queries from a previous jboss session are in running state reset to queue state.
		...
		try {
			dsLookupDao = DataSourceLookupDAOFactory.getDataSourceLookupDAO();
			 dataSourceLookupList = dsLookupDao.getDbLookupByHive("%");
	...
	public void runQueues() {
		log.info("ProcessQueue.class: runQueues()");
	//	log.debug("In ProcessQueue");
		QueryProcessorUtil qpUtil = QueryProcessorUtil.getInstance();
		int	transactionTimeout = this.readTimeoutPropertyValue(queue);
	...
	//		log.debug("My dslookupis ds conn info is " + dsLookupDao.getDataSource().getConnection().toString());
			List<DataSourceLookup> dataSourceLookupList = dsLookupDao.getDbLookupByHive("%");
	//		log.debug("I found this number of datasources: " +dataSourceLookupList.size() );
			while (true) {
				...
				String xmlRequest= StringUtils.EMPTY;
				String pmXml = StringUtils.EMPTY;
				Connection conn = null;
				PreparedStatement preparedStmt = null;
						...
						if (queryInstanceId != 0 && readTimeoutPropertyValue(queue) > 1) {
							...
							//	int waitTime = (readTimeoutPropertyValue(queue))* 1000;
								...
								try {
									...
									if (!exec.isJobCompleteFlag()) {
										String timeOuterror = "Result waittime millisecond <result_waittime_ms> :" +
												waitTime + " elapsed, setting to next queue after " + queue;
	...
	//	log.info("In set to next queue");
		DAOFactoryHelper daoFactoryHelper = new DAOFactoryHelper(dslookup.getDomainId(), projectId, ownerId);
		...
		QtQueryInstance queryInstance = queryInstanceDao.getQueryInstanceByInstanceId(Integer.toString(queryInstanceId));
		if (queryInstance.getBatchMode().equals(QueryManagerBeanUtil.ERROR))
			...
			List<QtQueryResultInstance> resultInstanceList = queryResultInstanceDao.getResultInstanceList(Integer.toString(queryInstanceId));
	...
	private int readTimeoutPropertyValue(String queueType) {
		QueryProcessorUtil qpUtil = QueryProcessorUtil.getInstance();
		String timeoutStr = StringUtils.EMPTY;
		int timeoutVal = 0;
		try {
			if (queueType.equals(QueryManagerBeanUtil.SMALL_QUEUE))
				timeoutStr = qpUtil.getCRCPropertyValue("edu.harvard.i2b2.crc.jms.small.timeoutsec");
			else if (queueType.equals(QueryManagerBeanUtil.MEDIUM_QUEUE))
				timeoutStr = qpUtil.getCRCPropertyValue("edu.harvard.i2b2.crc.jms.medium.timeoutsec");
			else if (queueType.equals(QueryManagerBeanUtil.LARGE_QUEUE))
				timeoutStr = qpUtil.getCRCPropertyValue("edu.harvard.i2b2.crc.jms.large.timeoutsec");
