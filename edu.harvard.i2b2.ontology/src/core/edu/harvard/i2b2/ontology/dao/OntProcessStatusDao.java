...
public class OntProcessStatusDao extends JdbcDaoSupport {
	...
	public OntologyProcessStatusType createOntologyProcessStatus(
		...
		try {
			...
			SaveOntProcessStatus saveOntProcessStatus = new SaveOntProcessStatus(dataSource, dbInfoType);
	...
	public OntologyProcessStatusType findById(int processId) {
		...
		return jt.queryForObject(sql, new geOntologyProcessStatuMapper(), processId);
	}
	...
	public List<OntologyProcessStatusType> findByProcessTypeAndStatus(String processTypeCd, String processStatusCd) {
		...
		return jt.query(sql, new geOntologyProcessStatuMapper(), processStatusCd,processTypeCd);
	}
	...
	public OntologyProcessStatusListType findProcessStatus(int processId, String processTypeCd, String processStatusCd, Date[] startDate, Date[] endDate, int maxReturnRow) {
		...
		String sql = "select " + topClause + " * from " + dbInfoType.getDb_fullSchema() + "ONT_PROCESS_STATUS   ";
	...
	public int updateStatus(final int processId, final Date endDate,
							final String processStateCd, final String statusCd) throws I2B2DAOException {
		final Timestamp today = new Timestamp(Calendar.getInstance().getTime().getTime());
		String sql = "update " + dbInfoType.getDb_fullSchema() + "ONT_PROCESS_STATUS set ";
		if (endDate != null)
			sql += "end_date = ?,";
		sql += "process_step_cd = ?,process_status_cd = ?,change_date = ?,status_cd = ? where process_id = ? ";
		int recordCount = 0;
		if (endDate != null)
			recordCount = jt.update(sql, endDate, processStateCd, statusCd, today, "U", processId);
		else
			recordCount = jt.update(sql, processStateCd, statusCd, today, "U", processId);
		return recordCount;
	}

	public int updateStatusMessage(int processId, String message) {
		String sql = "update " + dbInfoType.getDb_fullSchema() + "ONT_PROCESS_STATUS set  message = ? where process_id = ? ";
		return jt.update(sql, message, processId);
	}
	...
	public int updateCRCUploadId(int processId, String uploadId) {
		...
		return jt.update(sql, uploadIdInt, processId);
	}
	...
	private int getDeleteEditCount(GetReturnType returnType, DBInfoType dbInfo){
		...
		List<java.sql.Timestamp> queryResult;
		try{
	...
	public int createOntologyProcessType(String ontProcessType, String userId) throws I2B2DAOException {
		int numRowsAdded = 0;
		try {
			Date today = Calendar.getInstance().getTime();
			int processId = 0;
			String addSql = "insert into " + this.dbInfoType.getDb_fullSchema()
					+ "ONT_PROCESS_STATUS"
...
class geOntologyProcessStatuMapper implements RowMapper<OntologyProcessStatusType> {
	...
	@Override
	public OntologyProcessStatusType mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		if (startDate != null)
			processStatusType.setStartDate(factory.getXMLGregorianCalendar(startDate.getTime()));
		endDate = rs.getTimestamp("end_date");
		if (endDate != null)
			processStatusType.setEndDate(factory.getXMLGregorianCalendar(endDate.getTime()));
		processStatusType.setProcessStepCd(rs.getString("process_step_cd"));
		processStatusType.setProcessStatusCd(rs.getString("process_status_cd"));
